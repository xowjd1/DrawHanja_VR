...
namespace StixGames.NatureCore
{
    ...
    public class NatureMeshFilter : MonoBehaviour
    {
        ...
        [Delayed] [Tooltip("The max ratio at which edges will be considered equal.")]
        public float EdgeRatioThreshold = 1.1f;

        [Delayed]
        [Tooltip(
            "The smallest edge size, edges below this value will no longer be split. If you are having performance problems, try setting this to a higher value.")]
        public float MinEdgeSize = 0.1f;

        [Delayed]
        [Tooltip(
            "Having a very large amount of triangles will slow down grass fallback significantly. You can use this settings to stop normalization early.")]
        public float FallbackMinEdgeSize = 2.0f;

        [Delayed]
        [Tooltip("Terrain is split into smaller pieces for optimization. " +
                 "The size of these pieces is based on the max fade distance of your grass. " +
                 "In case the render fade distance is very small, this value limits the amount of meshes. " +
                 "There will be at most n * n pieces. So in case of 20, at most 400 meshes will be generated.")]
        public int MaxTerrainDivisions = 20;
        ...
        [SerializeField] private List<Mesh> fallbackMeshes;
        ...
        /// </summary>
        /// <returns></returns>
        public List<Mesh> GetMeshes(bool useFallbackMeshes = false)
        {
            CleanupInvalidMeshes();

            if (useFallbackMeshes)
            {
                if (fallbackMeshes == null || fallbackMeshes.Count == 0)
                {
        ...
        private void CleanupInvalidMeshes()
        {
            meshes?.RemoveAll(x => x == null);
            fallbackMeshes?.RemoveAll(x => x == null);
        }
        ...
        private void CreateRawMeshes(bool useFallbackMeshes)
        {
            ...
            if (meshFilter != null)
            {
                ...
                else
                {
                    if (useFallbackMeshes)
                    {
                        fallbackMeshes = new List<Mesh> { mesh };
                    }
                    else
                    {
        ...
        {
            var minGrassRenderDistance = GetComponents<GrassRenderer>()
                .Select(x => x.Material)
                .Where(x => x != null)
                .Min(GrassUtility.GetGrassFadeEnd);

            var terrainSize = Mathf.Min(terrain.terrainData.size.x, terrain.terrainData.size.z);
            var subTerrains = Mathf.Clamp(Mathf.CeilToInt(terrainSize / minGrassRenderDistance), 1, MaxTerrainDivisions);

            var terrainConverter = new TerrainRemesher(terrain);
            terrainConverter.SubmeshesPerSide = subTerrains;
