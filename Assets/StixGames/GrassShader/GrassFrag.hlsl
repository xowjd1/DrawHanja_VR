...
FragmentCommonData GrassFragmentSetup(half3 albedo, half3 specular, half smoothness, float3 worldPos, half3 viewDir, half3 worldNormal)
{
	...
	half3 diffColor = EnergyConservationBetweenDiffuseAndSpecular(albedo, specular, /*out*/ oneMinusReflectivity);
...
DeferredFramebuffers frag(FS_INPUT i)
{
	...
	// def.diffuseOcclusion = half4(0, 0, 0, 0);
	// def.specularSmoothness = half4(0, 0, 0, 0);
	// def.lighting = float4(gi.indirect.diffuse, 1);
