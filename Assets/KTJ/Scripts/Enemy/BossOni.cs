...
public class MoveToPlayerState : OniBossState
{
    ...
    public override void Update()
    {
        if (_skipFirstCheck) { _skipFirstCheck = false; return; }
...
public class MoveToPlayer2PhaseState : OniBossState
{
    ...
    public override void Update()
    {
        ...
        if (!_agent.pathPending && _agent.remainingDistance <= _oniBossStateMachine.AttackRange)
        {
            _agent.isStopped = true;
            _oniBossStateMachine.animator.SetBool("isWalking2Phase", false);

            float r = Random.value; // 한 번만 샘플링
            if (r < 0.33f)
    ...
    {
        _agent.isStopped = true;
        _oniBossStateMachine.animator.SetBool("isWalking2Phase", false); // ✅ 수정
        Debug.Log("NavMesh Move: Stop Walking (Phase2)");
    }
}
...
public class Boss2SmashAttackState : OniBossState
{
    ...
    public override void Enter()
    {
        ...
        // 사운드는 애니메이션 이벤트에서 Anim_PlaySmashSfx() 호출
    }
...
public class BossDieState : OniBossState
{
    ...
    public override void Enter()
    {
        ...
        _oniBossStateMachine.StartLoadNextSceneAfterDeath("Die", 0); // ✅ 페이드 → 인터스티셜 → 씬 전환
        Debug.Log("Boss Die");
    }
}
